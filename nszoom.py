...
def serialize_domain(domain):
    return b''.join(map(lambda label: pack(">B", len(label)) + label.encode('utf-8'), domain.split('.')))
...
class QuestionEntry:
    ...
    def serialize(self):
        return serialize_domain(self.domain) + pack(">HH", self.dns_type, self.dns_class)
...
class ResourceRecord:
    ...
    def deserialize(cls, data, offset):
        domain, offset = deserialize_domain(data, offset)
        dns_type, dns_class, ttl, rdlength = unpack(">HHIH", data[offset:offset + 10])
        rdata = decode_rdata(data, offset + 10, rdlength, deserialize_enum(TYPES, dns_type))
        return (ResourceRecord(domain, dns_type, dns_class, ttl, rdlength, rdata), offset + 10 + rdlength)

    def serialize(self):
        return serialize_domain(self.domain) \
               + pack(">HHIH", self.dns_type, self.dns_class, self.ttl, self.rdlength) \
               + encode_rdata(self.rdata, deserialize_enum(TYPES, self.dns_type))
...
def list_to_plain_object(prefix, list):
    return [("%s %d" % (prefix, i), list[i].to_plain_object()) for i in range(len(list))]
...
class Packet:
    def __init__(self, id, qr, opcode, aa, tc, rd, ra, rcode, questions, answers, authority, additional):
        self.id = id
        self.qr = qr
        self.opcode = opcode
        self.aa = aa
    ...
    @classmethod
    def form_request(cls, target, recursion=True, dns_type=None, dns_class='IN'):
        default_type, domain = get_domain_name(target)
        question = QuestionEntry(domain, TYPES[default_type] if dns_type is None else TYPES[dns_type],
                                 CLASSES[dns_class])
        return Packet(random.randint(0, 1 << 16), MESSAGE_TYPE['QUERY'], OPCODES['QUERY'],
                      0, 0,
    ...
    def deserialize(cls, data):
        id, options, questions_count, answers_count, authority_count, additional_count = unpack(">HHHHHH", data[:12])
        qr = options >> 15
        ...
        rcode = options & 0xF
        offset = 12
        questions, offset = deserialize_list(QuestionEntry, data, offset, questions_count)
        answers, offset = deserialize_resource_records(data, offset, answers_count)
        authority, offset = deserialize_resource_records(data, offset, authority_count)
        additional, offset = deserialize_resource_records(data, offset, additional_count)
        return Packet(id, qr, opcode, aa, tc, rd, ra, rcode, questions, answers, authority, additional)


    def serialize(self):
        options = self.qr << 15 | self.opcode << 11 | self.aa << 10 | self.tc << 9 | self.rd << 8 | self.ra << 7 | self.rcode
        header = pack('>HHHHHH', self.id, options, len(self.questions), len(self.answers), len(self.authority),
                      len(self.additional))
    ...
    def to_plain_object(self):
        ...
        if len(self.questions) > 0:
            sections.append(("QUESTIONS", list_to_plain_object("Question Entry", self.questions)))
        if len(self.answers) > 0:
            sections.append(("ANSWERS", resource_records_to_plain_object(self.answers)))
        if len(self.authority) > 0:
            sections.append(("AUTHORITY", resource_records_to_plain_object(self.authority)))
        if len(self.additional) > 0:
            sections.append(("ADDITIONAL", resource_records_to_plain_object(self.additional)))
        return sections


def get_default_dns_servers():
    if os.name == "nt":
        output = subprocess.check_output('ipconfig /all').decode(locale.getpreferredencoding())
        raw_servers = re.findall(r'(?:DNS Servers|DNS-серверы)\D+:([.\d\s]+)', output)
        servers = filter(lambda x: x != '', sum(map(lambda s: re.split('\s+', s), raw_servers), []))
        return list(servers)
...
def stringify_plain_object(obj, indent=0):
    ...
    for values in obj:
        if isinstance(values[1], list):
            result += "\t" * indent + "%s\n%s\n" % (values[0], stringify_plain_object(values[1], indent + 1))
        else:
            result += "\t" * indent + "%-10s: %s\n" % (values[0], values[1])
    return result
